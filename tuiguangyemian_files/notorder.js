// by goingta qq860180810 201704 chengdu
...
document.getElementById("not3orderid").value='no.'+myDate.getFullYear()+truedate+myDate.getDate()+myDate.getHours()+myDate.getMinutes()+myDate.getSeconds()+not3num;
...
var not3time=readcookie("not3time");
...
getci = GetRequesta(decodeURI(decodeURI(fromurl)));
  ...
  if (not3time != null && not3time != "") {
     document.write("<input type=hidden name=not3time value='"+not3time+"'>");
  }else{
     var not3time=getNowFormatDate();
     setCookie("not3time",not3time,30);
     document.write("<input type=hidden name=not3time value='"+not3time+"'>");
  }
